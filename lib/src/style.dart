...
abstract class CoreStyle {
  ...
  void elevation(double elevation,
    ...
    final offsetX = sin(angle) * elevation;
    final offsetY = cos(angle) * elevation;

    // custom curve defining the opacity
    var calculatedOpacity = (0.5 - (sqrt(elevation) / 19)) * opacity;
    if (calculatedOpacity < 0.0) calculatedOpacity = 0.0;

    final colorWithOpacity = color.withOpacity(calculatedOpacity);
